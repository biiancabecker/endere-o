...
public class EnderecoDao {
    public static boolean inserir(Endereco objeto) {
        String sql = "INSERT INTO endereco (bairro, longadouro, complemento) VALUES (?, ?, ?)";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
            ps.setString(1, objeto.getBairro());
            ps.setString(2, objeto.getLongadouro());
    ...
    public static void main(String[] args) {
        ...
        objeto.setBairro("Planalto");
        objeto.setLongadouro("Av. Brasil");
        objeto.setComplemento("1000");
    ...
    public static boolean alterar(Endereco objeto) {
        String sql = "UPDATE endereco SET bairro = ?, longadouro = ?, complemento = ?, codigo = ? WHERE codigo=?";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
            ps.setString(1, objeto.getBairro()); 
            ps.setString(2, objeto.getLongadouro());
